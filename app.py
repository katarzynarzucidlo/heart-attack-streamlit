...
import joblib\
\
st.set_page_config(page_title="Heart Attack Risk Predictor", page_icon="\uc0\u55358 \u57024 ", layout="centered")\
\
# -----------------------------\
...
\
st.title("\uc0\u55358 \u57024  Heart Attack Risk Predictor")\
...
with col2:\
    ...
    cp = st.selectbox("Chest Pain Type (0\'963)", options=[0, 1, 2, 3], index=2)\
    exang = st.selectbox("Exercise Induced Angina (0=No, 1=Yes)", options=[0, 1], index=0)\
    slope = st.selectbox("Slope of ST Segment (0\'962)", options=[0, 1, 2], index=2)\
    ca = st.selectbox("Number of Major Vessels (0\'963)", options=[0, 1, 2, 3], index=0)\
...
\
        ...
        st.exception(e)\
\
st.caption("Model file: `heart_attack_rf_pipeline.joblib`  \'95  App by Streamlit")\
}